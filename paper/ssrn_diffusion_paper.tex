...
\noindent Among marks the office granted, it is timing rather than
unusualness that predicts what happens next. Leading marks fail their first
use-proof gate --- the declaration, due between the fifth and sixth year,
that the mark is still in commercial use --- 1.4 percentage points more
often in the top within-class-cohort quintile than in the bottom, on a
52.3\% base, over 3.10 million registrations with class$\times$cohort fixed
effects, owner-clustered errors, and drafting and filer controls
($t = 8.3$). Almost all of it sits in the top quintile. The penalty is
episodic: carried back to the first scoreable cohorts it is two to three
times larger than anything since, vanishes for marks filed 2000--2004, and
returns from 2008, with the whole swing in technology classes and a small
steady penalty elsewhere. At the firm level the ordering runs the same way
and the \emph{lagging} tail stands out --- 0.44\% of the most lagging fifth
of registered debut owners later appear in SEC financial-reporting records,
against 0.24\% in the middle and 0.39\% of the most leading fifth.
Atypicality does none of this work: its association with SEC reporting is
indistinguishable from zero once the owner-to-registrant linkage spans the
whole corpus. Nor is the measure a proxy for patenting --- within firms the
association is under $0.06\sigma$ and flips sign between text
representations.

\noindent Two cautions. Read without conditioning on grant, the same data
support weaker and sometimes opposite conclusions, because registration is
itself selected on language. And scored on individual terms rather than
topic distributions, leading filings appear to belong to better firms ---
an association that dissolves or reverses under topic scoring, because term
scoring picks up lexical specificity rather than position. Text measures of
this kind can manufacture firm-performance correlations out of drafting
style. Code, panels, and the corpus build are public at
...
written in language the industry was already leaving behind, and their
owners are no more likely --- by the raw rates, somewhat less likely --- to
end up filing financial reports with the SEC. Being early is where the risk
is, and it is priced in the direction opposite to the one a reader would
guess.
...
are culled. Trademark law thereby supplies an unusually direct test: a
registered mark must be re-proved in use five to six years on or be
cancelled.
...
measurement hazard that applies to text-based novelty measures generally and
would have produced a more flattering paper had it gone unnoticed.
...
Three windows bound the samples, each set by an institutional fact.
...
(Table~\ref{tab:funnel}). Completing registration therefore mostly measures
whether the filer followed through to commercial use. The Section~8 gate is
...
This is about the examination funnel, not the market. Pooling granted and
abandoned filings mixes the two selections, and the resulting gradients are
attenuated and in places reversed (Appendix~\ref{app:uncond}).

The obvious objection is that unfamiliar language is simply refused more
often, so the measure recovers nothing beyond familiarity. Two features of
the shape rule that out. Completion falls at \emph{both} ends, so the most
...
axis: the signed axis, almost uncorrelated with it by construction, shows no
interior peak (Figure~\ref{fig:profiles}, panel C). What examination rewards
is neither novelty nor conventionality but a middling specificity ---
concrete enough to survive a definiteness objection, conventional enough to
avoid a likelihood-of-confusion refusal. The paper does not build on it.
...
Almost all of it sits in the top quintile, where the step from the fourth is
...
Raw pooled quintiles convey the direction: among 4.12 million registration
...
design-based against $+2.4$pp raw.
...
The series stops because the record does. The gate is close to a fixed
appointment rather than a hazard spread over years: of the 1.35 million
...
Two confounds could produce a spurious early result, and neither does.
...
among slower, against $-2.94$pp pooled. It is not an artifact of
already-failing owners being detected late. The restriction drops 61\% of
...
report for other reasons. Panel B of Figure~\ref{fig:outcomes} holds two
patterns that pull in different directions. The rate rises monotonically in
...
How owners are linked to public-market records bounds what the result can
mean. Owners are matched to SEC
...
The lagging end is the more striking. In raw rates, 0.444\% of the most
...
Two qualifications. The profile is a U rather than a slope, so a linear
...
So the two axes do different work. Atypicality predicts survival at the
use-proof gate but not which owners become SEC-reporting once composition is
absorbed. Lead is the timing bet, and it costs at both margins. Being early
is expensive; being unusual is not rewarded here.
...
So the claim is not orthogonality, which would overstate what is here, but
that no stable relationship exists in either direction: a reader wanting to
use trademark lead as a proxy for patenting would find nothing consistent to
lean on.

Two objections, and both fail. The first is timing: a patent is applied for
before its invention is in public use and a trademark when the firm is ready
...
twenty-fourth by coefficient and do not agree in sign with one another. The
classes reaching significance run against the complementarity story rather
than merely failing to support it: the only significant positives are
transport and cosmetics, neither a complement class, while the significant
negatives include medical devices and machinery, both of which are.

The sample makes the test harder, not easier. It is conditional on a
successful assignee match, so every owner patents at least once and the
...
The two axes bite at different points in a mark's life.
...
single specification, so the coefficients can be read down a column. Its
magnitudes differ from the section-specific tables above; the signs and the
ordering are what it is for.
...
gas and petroleum, artificial intelligence, and cloud computing. Entrants
are no better represented among the earliest carriers of a new theme than
among filers generally, so the arrival of new vocabulary is not an entrant
...
Four claims are defended. The first is the one the paper is for: vocabulary
position carries two separable signals, and it is the signed one that tracks
...
Filings are not independent draws, which bears on how any descriptive
display of this corpus should be read. Some of them share an exact position
...
What drives the episodic pattern remains unresolved. Cohort and gate year
...
two axes. Two qualifications. Topic scoring
...
Appendix~\ref{app:choices} reports the gate contrast under all six builds of
the corpus; every sign holds in each, and term scoring returns the larger
lead contrast, so the headline estimate is the conservative representation.
...
Appendix~\ref{app:choices} reports the calendar signature that leaves and
how far the two scorings agree; three consequences for the results follow,
...
Nothing here changes a sign or a conclusion --- only magnitudes, one
functional form, and the confidence with which magnitudes can be quoted.
...
inadequate the correction is an ordinary quadratic, so the curvature is not
evidence of a threshold or a multiplicative process.
...
including the never-registered. It is an inverse-U in \dkl{}
(middle-versus-tails depth $+1.4$pp, SE 0.1pp), because the composite
inherits the registration step's middle-favouring shape from
Figure~\ref{fig:outcomes}A. Unconditional survival analyses on this corpus
therefore recover an inverse-U whether or not the maintenance gate itself
favours the middle, which is why the body conditions on registration
throughout. Panel B shows the conditional gate outcome on the same filings:
mildly inverse-U with a clearly penalised leading tail, consistent in the
right tail with the cleaner single-gate cohort of
Figure~\ref{fig:s8pooled}, and a reminder that the left tail is
cohort-sensitive.
...
The two representations fail in opposite directions. Topic scoring is
