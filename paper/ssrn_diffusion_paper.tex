...
Three conventions run through every table below and are easier stated once.

\emph{Percentage points against a base.} Outcomes are rates, so a coefficient
is a shift in a probability: $+1.4$pp on a 52.3\% base means 53.7\% against
52.3\%, not 52.3\% multiplied by $1.014$.

\emph{Two uses of $\sigma$.} ``pp per $\sigma$'' means the outcome moves by so
many percentage points when the \emph{regressor} rises by one of its standard
deviations. A bare ``$\sigma$'', which appears only in the patenting
comparison, is the reverse: there the \emph{outcome} is standardized and the
regressor is in its own units. The two are not comparable.

\emph{Absorbing, composition, design-based.} Estimating ``within
class$\times$cohort'' means subtracting each cell's mean from every variable
before the regression, so only variation inside a single industry-and-year is
used and any difference between industries or years is removed rather than
modelled. \emph{Composition} is what that removes --- an apparent relationship
that is really a mix of industries or years differing in both score and
outcome. \emph{Design-based} describes estimates carrying the full set: fixed
effects, drafting and filer controls, one row per registration, and errors
clustered on the owner, as against \emph{raw} pooled rates.
...
per owner. The funnel below counts debut filings, the gate regressions unique
...
Reading only the Section~8 code would record every \S66(a) registration as a
survivor whatever became of it. That mistake puts a spurious $-46$pp
coefficient on the \S66(a) control; reading both codes moves it to $+10$pp,
since Madrid registrations in fact fail this gate more often than domestic
ones.
...
theme$\times$class$\times$cohort cells, where a filing's theme is the one
...
Assigning quintiles within class$\times$cohort rather than within the era
pool changes little. Assigning them within theme$\times$class$\times$cohort
...
technology class, raw quintile contrasts with 95\% intervals on settled
registrations. Three classes cross zero between 2001 and 2006; scientific
and computer services does not.}
\label{fig:techswing}
\end{figure}
