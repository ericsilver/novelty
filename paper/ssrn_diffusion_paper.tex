...
owners outperform the persistent middle ($\mu = 0.24$\%). Neither axis is a
proxy for patenting: within firms no association reaches a tenth of a standard
deviation, and each changes sign with the representation.

\noindent Two provisos. Atypicality can be measured on terms or on themes, and
the two disagree. Term atypicality predicts SEC reporting negatively
($-0.09$pp per $\sigma$) and patenting negatively ($-0.055\sigma$); theme
atypicality predicts neither on its own; and their disagreement --- each
significant with the sign opposite the other when both are entered --- is
itself the result, since it makes a text measure's sign at the firm level a
property of the representation rather than of the firm. The second proviso is
