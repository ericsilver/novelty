...
\begin{abstract}
\noindent Text-based novelty measures built on patents observe invention by
the small minority of firms that patent; trademark filings time-stamp
commercialization by everyone else, in plain commercial language. All 13.99
...
years after. Filings whose wording the industry subsequently moved toward are
\emph{leading}; those written in language it was already leaving behind are
\emph{lagging}.
...
\noindent None of it is large. Whether a firm hired a trademark attorney
predicts reaching SEC reporting far better than anything its language says:
0.83\% of represented first-time filers get there against 0.18\% of
self-filers, a coefficient some fifteen times any text effect here. What the
text adds is small, and it is also fragile in a specific and instructive way.
Scored on individual terms rather than on distributions over themes, leading
filings appear to belong to \emph{better} firms; entered together on the same
owners, the two scorings carry opposite signs and both are significant, so at
most one of them is measuring position rather than drafting style. Text
measures of this kind can manufacture firm-performance correlations, and a
change of representation is what exposes it. A third caution is structural:
read without conditioning on grant, the same data support weaker and sometimes
opposite conclusions, because registration is itself selected on language.
