...
Each result above is stated under one formulation chosen for legibility: one
...
\subsection{Representation, anchoring, weighting}
...
scored. The second row is the production scoring. The fourth row is the
legacy build; it differs from the production build in anchoring and
weighting at once, so the two per-filing term builds were constructed to
isolate each choice.
...
\footnotetext{The resolution row is computed on the gate sample rather than
the 6.01 million-filing common sample of the other rows.}
...
\subsection{Annual reference buckets}\label{app:rolling}
...
sees any of the language filed during its own year. The calendar signature
that leaves is reported below.

First, the gate penalty was overstated by about a third. The persisted
comparison is the software class on an identical 452{,}911-registration
sample: $+7.40$pp (SE $0.43$) under annual buckets against $+5.25$pp
(SE $0.42$) under per-filing windows, a ratio of $0.71$, with the
$t$-statistic \emph{rising} because the standard error falls further than
the estimate. The corpus-wide annual regression of the superseded build
returned $+2.15$pp (SE $0.30$) against the current $+1.39$pp, the same
ratio, and its per-split ratios ran $0.59$ to $0.73$; that run was not
persisted, so the corpus-wide statement rests on the superseded build's
log and the software-class artifact carries the claim.
...
Each new analysis in Sections~\ref{sec:life}--\ref{sec:secevents} was rerun
under its natural parameter toggles, on the identical data. Nothing
...
Ladder, counsel / self debuts & reporting lead $-0.21$pp & $-0.28$ / $-0.04$pp \\
Ladder, 2009--13 / 2014--18 & reporting atyp.\ $+0.11$pp & $-0.01$ / $+0.20$pp \\
\bottomrule
\end{tabular}
\end{center}
...
carries a third of it, which is Section~\ref{sec:eras}'s era swing seen
...
$-1.4$pp in advertising and retail, so it is reported as a bound for one
class rather than a general fact. All artifacts are under
\texttt{paper/v3/\_eval/}.

\subsection{Changing the time period}\label{sec:robust_periods}

The registration cohorts the gate estimates use begin in 2002 because that
is the conventional cut for this corpus, and end in 2018 because the
cancellation record runs to April 2026. Section~\ref{sec:eras} carries the
series back to the first scoreable cohorts: the penalty was two to three
times larger for marks filed in the late 1990s than for any cohort since,
reversed for filings made 2000--2004, and returned from 2008, with the swing
inside the four technology classes and, within them, in which themes each
era's leading filings were drawn from. Within the 2002--2018 window the
penalty is zero to negative for the 2002--2005 cohorts ($-2.3$pp for 2003)
and runs $+1.8$ to $+5.4$pp for every cohort from 2008 on the wide-window
raw series. The 2019 cohort, evaluable only on the narrow 6.5--7.0
observation band for registrations issued early enough in that year
(78{,}408), is consistent in sign at $+5.6$pp. Cohorts from
2020 have no observed gate. The pooled estimate is therefore an average over
a penalty that was not constant, and a reader who wants the penalty for a
period should read Figure~\ref{fig:cohort} rather than the pooled row.
...
atypicality correlates with the log of a filing's distinct term count at
$-0.681$ in class 009 and
$-0.620$ in class 035, for the reason set out in
...
weakens the coupling, taking the same correlations to $-0.217$ and
$-0.098$. The consequence for the lead is direct: the standard deviation
...
patterns diagnostic force (Appendix~\ref{sec:robust_hazard}). Across
...
registration curvature on this axis is not, and Section~\ref{sec:life}
reports only the unsigned version.}
\label{fig:s8pooled}
\end{figure}

\subsection{Term scoring at the firm level}\label{sec:robust_hazard}
...
Sections~\ref{sec:life}--\ref{sec:resolution} --- the era, theme, internet
