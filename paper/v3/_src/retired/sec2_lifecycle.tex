...
This section relates atypicality and lead to two outcomes on the largest
samples the record allows: registration, on every class-record (a filing
counted once per Nice class it claims) filed 1995--2018, and the first
use-proof gate, on every unique registration issued 2002--2018.
...
roughly nine in ten die by the applicant's own inaction: they never file the
statement of use, or they stop responding during prosecution. Only 4.3\%
are removed adversarially (Table~\ref{tab:funnel}). Registration is
therefore read as a launch indicator rather than an examiner's verdict.
...
per owner. A \emph{cohort} is the set of registrations issued in a calendar
year; where a split is instead by filing year, the text says so. The funnel
above counts debut filings, the gate regressions unique
...
years more conservative than the measured burn-in --- the early span where
thin reference windows inflate the scores --- requires
...
(Figure~\ref{fig:profiles}, panel C). The shape is the one audience-conformity
research predicts for evaluators \citep{Zuckerman1999, Hsu2006, Zhao2017},
here from an examiner and at census scale; \citet{TaeuscherRothe2020} find
settings where the optimum sits off-centre, and Section~\ref{sec:gate} shows
the market's gate has no interior optimum at all. The same shape appears on the
...
\begin{tabular}{lrrrr}
\toprule
 & Share of filings & Registration & Gate failure & Median terms (009 / 035) \\
\midrule
Represented & 73.3\% & 61.9\% & 47.5\% & 28 / 26 \\
Self-filed  & 26.7\% & 45.6\% & 69.2\% & 21 / 17 \\
\bottomrule
\end{tabular}
\end{center}

\noindent The gate gap is the larger of the two, and it is not a drafting
effect: the gate is proof of continued use, so a self-filed registration
that fails it names a product that went out of commerce within six years,
and self-filers are disproportionately new and small.
...
further ahead of it. Counsel of record therefore moves both sides of every
...
Who drafts changes the extent but not the direction: owners who hire
...
at the bottom theme-\dkl{} quintile to 52.4\% at the top, positive in 33
...
uppercased, stripped of punctuation and of two dozen legal suffixes, so
...
survival is $0.38$ pooled across classes ($0.42$ in the single-class checks
where it was first measured), so the 1.27 million owner clusters, not the filings,
...
\caption{Event-dated first-gate failure lift (theme \dkl{} top minus
...
bars indicate the leading penalty. Whiskers are
two-sample binomial intervals, uncorrected for the within-owner correlation
($0.38$) and so narrower than clustered intervals; read
Table~\ref{tab:gate_lpm} for precision. Per-class breakouts are in the
online appendix.}
\label{fig:forest}
\end{figure}

Within the 2002--2018 window the penalty looks modern: absent or negative
for the 2002--2005 cohorts, turning positive in 2006, and running
...
$+6.9$pp for the 1996 cohort and rises to $+10.1$pp for 1999 --- roughly
twice the largest cohort contrast observed since --- then collapses. The 2002--2005
...
and assigns quintiles within the era (raw),
...
registrations from those filing years failed at 64.9\% against 47.5--59.0\%
in the other large themes. That one theme is most of the late-1990s
penalty. In 2000--2004 it held
...
registrations fail at 67.2\% over the whole period, the highest base rate
of any theme, and their within-theme lead contrast pooled across cohorts,
$+8.4$pp, is the largest of any theme (the table's 2008--2014 cell, cut
within that era alone, is $+3.3$pp). The
...
apparatus ($+9$pp), with negative lifts in beer and soft drinks, transport
and construction. These are the extremes of a 44-class screen of raw
contrasts: the largest of many noisy estimates sit away from zero by
selection, so the named classes locate where the penalty concentrates and
their magnitudes should be read with that in mind.

The risk persists at the year-ten renewal, and on the design-based
estimates does not attenuate. Table~\ref{tab:staged} models survival, so its
signs run opposite to the failure convention used above: lead enters at
$-0.49$pp on passing the first gate and $-0.63$pp on renewing at the
second --- the same penalty, stated on the other side. What attenuates is
...
($\Delta$AIC $= 83$, the largest margin anywhere in this paper). The fitted shape is an inverted V: renewal is highest just below the
centre of the lead axis (64.2\% near $z = -0.4$) and lowest at both
tails, 45.5\% at the most lagging bin --- the lowest anywhere on the
axis --- and 50.6\% at the most leading. The
...
leading tail. Among first-gate survivors, the most lagging renew least of all. Most of
the difference between leading and lagging resolves at first proof of use;
what remains at renewal is a penalty on both extremes of the lead axis,
the lagging one the larger.
...
slightly \emph{less}. The lead penalty is present in both halves, stated
here on passing, as the paragraph's outcome is: $-3.48$pp
...
not. Internet-bearing filings are 16.9\% of scored registrations; the
per-class shares below are of class-records filed 1995--2018:
...
The penalty has a common shape in event time. Dating the internet's arrival
in each class as the first filing year in which the pattern reaches 2\% of
the class's filings (1994--1995 in the technology and transport classes,
1999--2000 in apparel and chemicals, 2008 in machinery), and aligning each
class's per-cohort gap on years since its own arrival: the gap between
internet-bearing registrations and the rest starts near $+7$ points in the
first observable cohorts, declines to about zero nine to eleven years after
arrival, and settles between $+1$ and $+4$ (Figure~\ref{fig:webshape}).
Across the eight classes observable in both phases, the mean gap is $+4.4$
points in years zero to four and $+1.6$ from year ten, shrinking in six of
the eight. Calendar time disguises this: transport, whose arrival was 1995,
...
own early years. The class's clock starts when the vocabulary arrives, not
when the internet did. Clothing is the standing exception: its
...
\caption{The internet gap in first-gate failure, inside each class.
\emph{Left:} internet-bearing minus other registrations' failure rate, by
registration cohort in years since the internet pattern reached 2\% of the
class's filings, averaged across classes (band: one standard deviation
across classes). \emph{Right:} five classes in calendar time. Cells with
fewer than 100 registrations in either group are dropped.}
\label{fig:webshape}
\end{figure}
...
as a success. It is not. Among 2.28 million abandoned applications (filings
1995--2015, so every abandonment has a full six-year refile window), only
...
The refilings that do occur were shown in Section~\ref{sec:amend} to be
rewritten toward the class's typical description, so the atypicality of
registered marks is compressed relative to what applicants first wrote,
and every estimate on registered marks is an estimate on conformed
language. For the 42{,}115 refiled cases the first text is observed; for an
applicant who conformed before ever filing it is not, and that part of the
compression cannot be bounded. The refiled cases themselves move nothing:
they are $1.1$\% of the gate sample, they fail it slightly less often than
other registrations (51.7\% against 52.8\%), and the within-class-and-year
contrast is $+2.25$pp with them and without them. Which of the two texts
predicts their gate cannot be told on 34{,}551 cases ($+0.9$pp on the
rewrite, $+0.7$pp on the original, both with standard errors near
$0.8$pp).
...
likelihood-of-confusion refusal. The endpoints are the inverse-U of
Section~\ref{sec:registration}; Appendix~\ref{app:registration} gives the
profile on each axis and why familiarity alone cannot produce it.
