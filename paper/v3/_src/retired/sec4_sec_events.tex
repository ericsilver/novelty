...
(0.44\%) appear in SEC reporting records at any time, and 1{,}795
(0.18\%) registered a class of securities or completed an initial public
offering at any time. The funded rung requires the round to postdate the
debut; the reporting and listing rungs do not, so owners already reporting
before their first trademark filing are counted, and the section's staged
analysis shows they are a majority of the exchange-listed. The rungs nest: 7.3\% of funded owners reach reporting against
...
\citep{Block2014}; vocabulary position does not, which sharpens the null
rather than weakening it. Reporting and listing do vary, and in the direction
...
53.6\%, five hundred hold 82.9\%, and a thousand hold 92.6\%. Revenue among linked firms is concentrated in a few hundred owners.
...
within about $0.1$ of the average on both. Weighting every linked firm by its
revenue, the linked set as a whole sits $0.02$ standard deviations below
...
learning, deep learning, neural networks, natural language processing and
understanding, computer vision, predictive analytics, chatbots, generative
...
smart contracts). Counted as class-filing rows over the full corpus, with a
multi-class filing counted once per class, internet vocabulary appears in
2.94 million rows, AI in 167 thousand, blockchain in 114 thousand.
...
abandoned before use. 31.6\% of internet registrations carry a first-gate cancellation against
6.5\% of AI registrations and 6.8\% of blockchain ones; these denominators
include registrations too young to have faced the gate, which is why the AI
and blockchain figures are low, and they are not survival rates.
...
atypicality gradient is sensitive to exactly that. The lead coefficient is
unchanged under alternative linkages; the atypicality coefficient is not,
so only the lead result is robust to the name-match design. The financing
...
In raw rates, computed on the rebuilt owner--EDGAR crosswalk, 0.879\% of the
most lagging fifth of registered debut owners appear in SEC reporting
records, against 0.486\% in the middle fifth and 0.691\% of the most leading
fifth: both tails beat the middle, and the lagging tail beats the leading
one. The rebuild roughly doubles every level relative to the earlier
crosswalk without changing the shape. Under the design-based specification the profile declines from the
...
points; Section~\ref{sec:robust} fits it: a V with its vertex about a
...
firm, estimated on 94{,}181 firm-years across 17{,}506 firms under theme
scoring (93{,}551 and 17{,}415 under term scoring) --- 134{,}075 matched
firm-years before dropping the firm-years that contribute no within-firm
variation. The outcome is
...
nearly the same number attached to different quantities. Only those two
cells are established; the mirror's other diagonal is within noise, and the
orthogonality conclusion rests on the magnitude bound --- every cell under
$0.06$ standard deviations --- rather than on any cell's sign.
...
about $0.18$ standard deviations; the between-firm correlation of firm
means is $-0.020$, and the raw firm-year correlation $+0.010$. The weighting of the reference window is irrelevant to all of it:
scoring against an exponentially decayed reference instead of a flat one moves
the contemporaneous coefficients by less than $0.004$.
...
autocorrelated within firm, gives $-0.054$ to $-0.089$ standard deviations of log patenting under theme
scoring and $+0.012$ to $+0.024$ under term scoring. No ordering isolates a
direction: the offsets are flat within each scoring and opposite in
...
services classes substitutes, the rest neither. The sectoral build differs
from the pooled grid: it is scored at 200 themes, keeps firms with at least
three panel years, and requires 50 firms and 300 firm-years per cell.
Estimating inside each group gives $-0.000$
...
non-monotone profile, not the quintile contrast of Table~\ref{tab:edgar}). Surprising filings register less often; among granted marks the market does
...
A linear coefficient cannot represent the inverse-U documented in Section~\ref{sec:registration}, where
...
relative ($t = 3.8$); the row is exploratory pending the Form~D
re-extract. Atypical language is invisible at the financing stage
...
reference window. Under annual reference buckets the entry row read as
active negative selection. Per-filing
...
zero ($-0.004$pp, SE $0.007$). Leading filings fare worse at every stage where the
