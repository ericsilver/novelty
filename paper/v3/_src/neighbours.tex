...
step from unusual language to devalued competence --- and because the label
itself is unsettled: a bibliometric review of 4{,}407 articles finds
\emph{radical}, \emph{breakthrough}, \emph{discontinuous} and
\emph{disruptive} used with definitions that are inconsistent within labels
and not distinctive across them \citep{Verhoeven2025}.

\emph{Atypicality in the bibliometric sense} \citep{Uzzi2013}, applied to
patents as \emph{unconventionality} \citep{BerkesGaetani2021}, scores the
...
the rarity of its internal pairings --- Section~\ref{sec:definitions}
shows the two are close to independent on this corpus, and the paper's
combination measures are the Uzzi-sense quantity. The word is kept
because both mean unlike the population; which population, and along what,
differ.
...
breadth of sources behind a patent, breadth of influence ahead of it ---
where $K^-$ and $K^+$ measure the distance of a filing's language from
its own class's, before and after. Generality also requires the forward
citations to have arrived, so it observes realized influence; $K^+$
observes the class's movement whether or not the filing influenced it.
