...
Both windows are anchored on the filing's own date --- per-filing windows,
as against one shared reference per class and calendar year --- spanning
the 1,826 days before it
...
because its software-class lead contrast sits at the low end of the
resolutions tried (only the 100-theme refit returns less).
...
and had not been writing that way in the five years before. The symbol
\dkl{} is used interchangeably with $L$; Table~\ref{tab:constructs} maps
the terms.
...
\citep{Fleming2001, Uzzi2013}: familiar parts, new adjacency. Filings in the top fifth of that share fail the gate $6.1$
