...
For each (owner_name, filing_year):
    ...
    mean_dkl            float   mean kl_vs_past - kl_vs_future  (clean)
    ...
    mean_pros_kl        float   mean kl_vs_past   (prospective KL)      (clean)
    mean_retr_kl        float   mean kl_vs_future (retrospective KL)    (clean)
...
def aggregate(surprise: pl.DataFrame, *, high_dkl: float = 0.5) -> pl.DataFrame:
    ...
    return (
        ...
        .agg(
            ...
            pl.col("kl_vs_past").filter(pl.col("clean")).mean().alias("mean_pros_kl"),
            pl.col("kl_vs_future").filter(pl.col("clean")).mean().alias("mean_retr_kl"),
