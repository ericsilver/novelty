...
def main() -> int:
    ...
    firm = universe.group_by("owner_name").agg(
        ...
        pl.col("survived_5y").mean().alias("mean_surv5"),
    ...
    panels = [
        ...
        ("mean_surv5", "mean", "viridis", None,
         "Mean Renewed-5y rate per firm"),
        # Firm-level "consistency": std of survived_5y across the firm's filings.
        # 0 if every filing has the same outcome; 0.5 max when half register, half don't.
        ("mean_surv5", "std", "magma", None,
         "Within-firm std.\\ of 5y survival outcomes"),
