"""Variant of templated_survival_v2 that DROPS the n_terms >= 3 clause
from CLEAN.  Lets the short commodity templates ("restaurant services",
"legal services") into the cohort so we can compare survival between the
two filter regimes.

Reuses data/processed/gs_template_counts.parquet.

Output:
  paper/results/templated_survival_no_nterms.png
  paper/results/templated_survival_no_nterms.json
"""
...
def main() -> int:
    print("[load] gs_template_counts.parquet (cached, global)…", flush=True)
    ...
    print(f"\n  total clean (no-n_terms) filings tagged: {n_total:,}")

    print("\nFiling-level outcomes by template-multiplicity bin (no n_terms filter):")
    rows = []
    ...
    # Top-50 templates with their no-n_terms outcomes
    print("\n[top50] computing outcomes for the top-50 templates (no n_terms filter)…", flush=True)
    ...
    for path in sorted(PROC.glob("outcomes_class*.parquet")):
        ...
        if not cls.isdigit():
            continue
        out = (pl.read_parquet(path).filter(CLEAN_NO_NTERMS)
               .with_columns(
                   (pl.col("reached_registration") & pl.col("currently_live")).alias("passed_5y")
               )
               .select("serial_number", "reached_registration", "passed_5y"))
    ...
    print("\nTop-50 most-templated normalised G/S strings (no n_terms filter):")
    ...
    ax.set_title("Templated trademark descriptions: registration \\& still-live rate\n"
                 "WITHOUT $n_\\mathrm{terms}\\geq 3$ filter "
                 f"(filing years 1990--2018; n={n_total:,} filings)")
    ax.legend(loc="lower left")
    ax.grid(alpha=0.3, axis="y")
    fig.tight_layout()
    out_png = RESULTS / "templated_survival_no_nterms.png"
    fig.savefig(out_png, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"\n[done] wrote {out_png}")

    out_json = RESULTS / "templated_survival_no_nterms.json"
